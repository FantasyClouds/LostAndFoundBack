...
export default {
  ...
  async mounted() {
    try {
      this.loading = true;
      console.log("amisRequire 在这之后加载"); // 检查点
      const amisRequire = await loadSDK();
      console.log("amisRequire 是否存在?", !!window.amisRequire); // 检查点
    // ...后续逻辑
      const scoped = amisRequire("amis/embed");
      const { normalizeLink } = amisRequire("amis-core");
      const router = this.$router;
      
      console.log("Router实例是否存在:", !!this.$router); // 应为 true
